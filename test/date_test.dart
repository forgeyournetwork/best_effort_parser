  ...
  group('NameParser', () {
    group('.parse(String input)', () {
      test('handles singular compact dates with default settings', () {
        expect(DateParser.basic().parse('1/2/2000'),
            equals([ParsedDate(2000, 1, 2)]));
        expect(DateParser.basic().parse('12/31/1999'),
            equals([ParsedDate(1999, 12, 31)]));
      });
      ...
      test('handles multiple compact dates with the default settings', () {
        ...
        expect(
            DateParser.basic().parse('1/3/1803-2/4/2020-10/3/2400'),
            equals([
              ParsedDate(1803, 1, 3),
              ParsedDate(2020, 2, 4),
              ParsedDate(2400, 10, 3)
            ]));
      });

      test('handles singular normal dates', () {
        expect(DateParser.basic().parse('2 January 2010'),
            equals([ParsedDate(2010, 1, 2)]));
        expect(DateParser.basic().parse('Jul 2nd, 2005'),
            equals([ParsedDate(2005, 7, 2)]));
      });
      ...
      test('handles multiple normal dates', () {
        ...
        expect(
            DateParser.basic().parse(
                'From the 1st of January, 1999, to the 2nd of March, 2000, '
      ...
      test('creates output using as much info as it has', () {
        ...
        expect(DateParser.basic().parse('1900-2000'),
            equals([ParsedDate(1900), ParsedDate(2000)]));
        expect(DateParser.basic().parse('January 2010'),
            equals([ParsedDate(2010, 1)]));
      ...
      test('has default handling of seasons', () {
        expect(DateParser.basic().parse('Spring 2000'),
            equals([ParsedDate(2000, 3)]));
        expect(DateParser.basic().parse('Summer 2000'),
            equals([ParsedDate(2000, 6)]));
        expect(DateParser.basic().parse('Fall 2000'),
            equals([ParsedDate(2000, 9)]));
        expect(DateParser.basic().parse('Winter 2000'),
            equals([ParsedDate(2000, 12)]));
      });
      ...
      test('returns an empty list if the input cannot be parsed', () {
        ...
        expect(samples.map((s) => DateParser.basic().parse(s)),
            everyElement(equals(<ParsedDate>[])));
      });

      test(
          'adds and removes digits as needed to standardize the numerical representation',
          () {
        expect(DateParser.basic().parse('0001/00002/90'),
            equals([ParsedDate(1990, 1, 2)]));
        expect(DateParser.basic().parse('0002-0003 January 2019'),
            equals([ParsedDate(2019, 1, 2), ParsedDate(2019, 1, 3)]));
        expect(
            DateParser.basic().parse('1/1/1 to 10/10/10 to 11/11/11'),
            equals([
              ParsedDate(2001, 1, 1),
              ParsedDate(2010, 10, 10),
              ParsedDate(2011, 11, 11)
            ]));
      });
      ...
      test('can have custom digit suffixes', () {
        expect(
            DateParser.basic(digitSuffixes: 'foo').parse('1foo january 2000'),
  ...
  group('ParsedDate', () {
    ...
    group('diagnosticString', () {
      test('only includes values when it should', () {
        ...
        expect(ParsedDate(2000, 10).diagnosticString(),
            '[Month]: 10 [Year]: 2000');
        expect(ParsedDate(2000, 10, 10).diagnosticString(),
            '[Day]: 10 [Month]: 10 [Year]: 2000');
      });

      test('works with optional parameters', () {
        expect(
            ParsedDate(1, 1, 1).diagnosticString(
                separator: '', dayLabel: '', monthLabel: '.', yearLabel: '.'),
            '1.1.1');
      });
    });
  });
